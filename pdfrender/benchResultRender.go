...
func showTestBlock(tests []data.TestBenchType, title string) {
	...
	for i, test := range tests {
		...
		for _, testResults := range test.Results {
			...
			case "fail":
				failTextBlock(pdf.GetX(), y, columnStatusW-padding, testResults.Status, true)
			case "warn":
				warnTextBlock(pdf.GetX(), y, columnStatusW-padding, testResults.Status, true)
			case "pass":
				passTextBlock(pdf.GetX(), y, columnStatusW-padding, testResults.Status, true)
			default:
				infoTextBlock(pdf.GetX(), y, columnStatusW-2*padding, testResults.Status, true)
			}
